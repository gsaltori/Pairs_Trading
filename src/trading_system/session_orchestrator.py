"""
Session Strategy Production Orchestrator

Asia Range → London Expansion trading system.

Coordinates:
- Session Engine (Asia range calculation, London entry)
- Gatekeeper (structural filter)
- Risk Engine (position sizing, governors)
- Execution Engine (MT5 orders)

FILTER ORDER:
1. Session engine calculates Asia range and detects breakout
...
from .session_engine import SessionEngine, SessionSignal, SessionDirection
...
class SessionTradingSystem:
    ...
    def initialize(self) -> bool:
        """Initialize system."""
        self.logger.info("=" * 60)
        self.logger.info("SESSION TRADING SYSTEM")
        self.logger.info("Strategy: Asia Range → London Expansion (R=2.5)")
        self.logger.info("=" * 60)
    ...
    def run(self) -> None:
        ...
        while self._running:
            try:
                self._process_cycle()
                time.sleep(30)  # Check every 30 seconds for session timing
    ...
    def _process_cycle(self) -> None:
        ...
        if self._bar_count % 24 == 0:  # Every ~12 hours for M30
            self.risk_logger.log_state(self.risk_engine.state)
    ...
    def _process_signal(self, signal: SessionSignal) -> None:
        """Process session signal."""
        symbol = self.config.strategy.primary_symbol
        
        self.logger.info(
            f"Session Signal: {signal.direction.value} @ {signal.entry_price:.5f}, "
            f"Asia: {signal.asia_range_pips:.0f} pips, Bias: {signal.asia_bias.value}"
        )
    ...
    def _create_trade_signal(self, session_signal: SessionSignal) -> TradeSignal:
        """Convert SessionSignal to TradeSignal."""
        direction = SignalDirection.LONG if session_signal.direction == SessionDirection.LONG else SignalDirection.SHORT
        
        return TradeSignal(
            bar_index=0,
            timestamp=session_signal.timestamp,
            direction=direction,
            entry_price=session_signal.entry_price,
            stop_loss=session_signal.stop_loss,
            take_profit=session_signal.take_profit,
            atr_value=session_signal.asia_range_pips * 0.0001,  # Approximate
        )
    ...
    def _shutdown(self) -> None:
        ...
        self.logger.info(f"Sessions analyzed: {self.session_engine.sessions_analyzed}")
        self.logger.info(f"Valid setups: {self.session_engine.valid_setups}")
    ...
    def get_status(self) -> dict:
        """Get system status."""
        asia = self.session_engine.get_current_asia_range()
        
        return {
            "running": self._running,
            "strategy": "Asia → London Expansion (R=2.5)",
            ...
            "current_asia": {
                "high": asia.high if asia else None,
                "low": asia.low if asia else None,
                "range_pips": asia.range_pips if asia else None,
                "bias": asia.bias.value if asia else None,
            } if asia else None,
...
def main():
    """Entry point."""
    print("=" * 60)
    print("SESSION TRADING SYSTEM")
    print("Strategy: Asia Range → London Expansion (R=2.5)")
    print("=" * 60)
    print()
